...
static void sdma_channel_attach_bd(struct SDMA* sdma, int channel, int bdnum)
{
	...
	if (!channel) {
		xaf_malloc(&sdma->bd0, PAGE_SIZE, 0);
		memset(sdma->bd0, 0, PAGE_SIZE);
	}
	else {
		xaf_malloc(&sdma->chan_info[channel].bd, sizeof(struct sdma_buffer_descriptor) * bdnum, 0);
